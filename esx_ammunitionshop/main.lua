...
AddEventHandler('esx_duty:hasEnteredMarker', function (zone)
  ...
  if zone == 'InsuranceDuty' then
    CurrentAction     = 'insurance_duty'
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  end
end)
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if CurrentAction == 'taxi_duty' then
        ...
        sendNotification(_U('not_taxi'), 'error', 5000)
        Wait(1000)
      end 
    end

    if CurrentAction == 'insurance_duty' then
        if PlayerData.job.name == 'insurance' or PlayerData.job.name == 'offinsurance' then
          TriggerServerEvent('duty:insurance')
        ...
        sendNotification(_U('not_insurance'), 'error', 5000)
        Wait(1000)
      end 
    end
        ...
        if CurrentAction == 'police_duty' then
          ...
          sendNotification(_U('not_police'), 'error', 5000)
          Wait(1000)
          end
        end
      end
